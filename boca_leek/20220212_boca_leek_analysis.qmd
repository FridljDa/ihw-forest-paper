...
alpha <- 0.05
nodedepth_forest <- 7
nbins_quantile <- 2^nodedepth_forest
```

```{r}
ihw_quantile_n <- IHW::ihw(p ~  N,
                           data = BMI_GIANT_GWAS, 
                           lambdas = Inf,
                           alpha = alpha, 
...
```{r}
lengths <- lapply(ihw_forest_n_maf@weight_matrices_forest, function(weight_matrices_forest_fold){
      sapply(weight_matrices_forest_fold, length)
    })

median(unlist(lengths))
```
...
```{r}
rejections(ihw_forest_all)
```

```{r}
save.image(paste0(Sys.Date(),"_boca_leek_analysis.RData"))
```
