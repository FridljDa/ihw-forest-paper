...
## Quantile
```{r}
groups_by_filter_multivariate <- function(covariates_train, covariates_test, nbins) {
  ...
  groups <- lapply(seq_len(nvar), function(i) {
    covariate_train_i <- data_train[, i, drop = TRUE]
    covariate_test_i <- data_test[, i, drop = TRUE]
    
    breaks <- quantile(covariate_train_i, probs = seq(0, 1, length.out = nbin_dim), na.rm = TRUE)
    covariate_test_i_binned <- cut(covariate_test_i, breaks, include.lowest = TRUE)
  })
  browser()
