...
table(between(pvals_bmi, s0, 1-s0))/length(pvals_bmi)
table(between(pvals_estrogen,s0, 1-s0))/length(pvals_estrogen)
```
...
```{r, cache=TRUE}
res_3_glm <- adapt_glm(x = x_bmi, pvals = pvals_bmi, pi_formulas = formulas,
                 mu_formulas = formulas, nfits = nfits, alphas =alpha,
                 s0 = rep(0.3, length(pvals_bmi)))
res_3_glm$nrejs
```
So at alpha = 0.1, 0 rejections
at alpha = 0.2, 0 rejections
at alpha = 0.3, 582 rejections

```{r}
par(mfrow = c(3, 1))
adaptMT::plot_1d_thresh(res_3_glm, x_bmi, pvals_bmi, 0.3, "P-value Thresholds (alpha = 0.3)",
               disp_ymax = 0.5)
adaptMT::plot_1d_thresh(res_3_glm, x_bmi, pvals_bmi, 0.2, "P-value Thresholds (alpha = 0.2)",
               disp_ymax = 0.5)
adaptMT::plot_1d_thresh(res_3_glm, x_bmi, pvals_bmi, 0.1, "P-value Thresholds (alpha = 0.1)",
               disp_ymax = 0.5)
```
