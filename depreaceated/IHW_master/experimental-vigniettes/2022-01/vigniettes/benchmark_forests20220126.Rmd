...
```{r create_fdp_eval}
fdp_eval_list <- function(IHW_results, Hs) {
...
betamix_sim <- foreach(i=1:nrow(betamix_sim_combs)) %dorng% {
  IHWStatsPaper::beta_unif_sim(m=m, mus_slope=betamix_sim_combs$mu_slope[i], one_sided_tests=FALSE, prob_one_sided = 0.25)}
```

```{r, eval = T}
ihw_betamix_sim_forest <- foreach(betamix_sim_i = betamix_sim) %dopar% {
  Ps_i <- betamix_sim_i$Ps
  Xs_i <- as.matrix(betamix_sim_i$Xs)
  ihw_betamix_sim_forest_i <- ihw(Ps_i, Xs_i, alpha = alpha, folds = folds, strat = "RFCDE", ntrees = ntrees, tau = tau, lambda = lambda)
}
```
...
```{r, eval = T}
evaluated_betamix_sim_forest <- foreach(i = 1:nrow(betamix_sim_combs)) %dorng% {
  ihw_betamix_sim_forest_i <- ihw_betamix_sim_forest[[i]]
  evaluated_betamix_sim_forest_i <- IHWStatsPaper::fdp_eval(Hs = betamix_sim[[i]]$Hs, rjs =IHW::rejected_hypotheses(ihw_betamix_sim_forest_i))
  dplyr::mutate(evaluated_betamix_sim_forest_i,
    mu_slope = betamix_sim_combs$mu_slope[i],
    method = "IHW-Forest", 
    m =m
  )
}

#evaluated_betamix_sim_forest <- bind_rows(evaluated_betamix_sim_forest)
saveRDS(evaluated_betamix_sim_forest, file= "huber_github/precomputed_results/evaluated_betamix_sim_forest.Rds")
```

```{r, eval = F}
evaluated_betamix_sim_forest <- evaluated_betamix_sim_forest %>%
  group_by(mu_slope, method) %>%
  summarize(
    FDR = mean(FDP),
    Power = mean(pow),
    n_monte_carlo = n(),
...
fdp_eval_list(ihw_betamix_slic, Hs)
##      rjs        pow        FDR
## 1 103.39 0.09449784 0.07155839
```

Now we run IHW with forest. 
```{r run_IHW_forest, message=FALSE, eval = F}
...
fdp_eval_list(ihw_betamix_forest, Hs)
##      rjs       pow        FDR
...
fdp_eval_list(ihw_betamix_RFCDE, Hs)
```
...
fdp_eval_list(ihw_betamix_noise_BocaLeek, Hs)
##      rjs       pow        FDR
...
fdp_eval_list(ihw_betamix_noise_RFCDE, Hs)
##      rjs       pow        FDR
## 1 471.17 0.4225245 0.08725322
```
