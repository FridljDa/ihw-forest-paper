...
fdp_eval_ihw <- function(strat, ihw_res, sim_par = NULL, sim = NULL, Hs = NULL) {
  ...
  evaluated_ihw <- cbind(evaluated_ihw, sim_par)
...
eval_sim_sum_power <- ggplot(eval_betamix_sum, aes_string(x = column, y = "Power", shape = "strat", col = "strat")) +
  geom_line() +
  geom_point() +
  theme_cowplot()
...
columns <- c("strat", "m", "K_coarse")
eval_group_sim_sum <- eval_group_sim %>%
  group_by_at(vars(one_of(columns))) %>%
  summarize(
    FDR = mean(FDP, na.rm = TRUE),
    Power = mean(pow, na.rm = TRUE),
...
saveRDS(eval_group_sim_sum, file = "huber_github/precomputed_results/evaluated_grouped_sim_forest.Rds")
```

```{r}
eval_group_sim_sum_FDR <- ggplot(eval_group_sim_sum, aes(x = K_coarse, y = FDR, shape = strat, col = strat)) +
  geom_line() +
  geom_point() +
  geom_hline(yintercept = alpha, linetype = 2) +
  theme_cowplot()

eval_group_sim_sum_power <- ggplot(eval_group_sim_sum, aes(x = K_coarse, y = Power, shape = strat, col = strat)) +
  geom_line() +
  geom_point() +
  theme_cowplot()

ggarrange(eval_group_sim_sum_FDR, eval_group_sim_sum_power, nrow = 1, widths = c(1, 1), common.legend = TRUE, legend = "bottom")
```
...
```{r}
Ps <- lapply(betamix_sim, function (betamix_sim_i) betamix_sim_i$Ps)
ihw_betamix_quantile <- calc_ihw(strat = "quantile", Ps = Ps, Xs = Xs_binned, alpha = alpha, folds = folds, ntrees = ntrees, tau = tau, lambda = lambda)
...
columns <- c("strat", "mu_slope")
eval_betamix_sum <- eval_betamix_sim %>%
  group_by_at(vars(one_of(columns))) %>%
  summarize(
    FDR = mean(FDP, na.rm = TRUE),
    Power = mean(pow, na.rm = TRUE),
...
saveRDS(eval_betamix_sum, file = "huber_github/precomputed_results/evaluated_betamix_sim_forest.Rds")
```

Now we run IHW with forest. 
```{r}
eval_betamix_sum_FDR <- ggplot(eval_betamix_sum, aes(x = mu_slope, y = FDR, shape = strat, col = strat)) +
  geom_line() +
  geom_point() +
  geom_hline(yintercept = alpha, linetype = 2) +
  theme_cowplot()

eval_betamix_sum_power <- ggplot(eval_betamix_sum, aes(x = mu_slope, y = Power, shape = strat, col = strat)) +
  geom_line() +
  geom_point() +
  theme_cowplot()

ggarrange(eval_betamix_sum_FDR, eval_betamix_sum_power, nrow = 1, widths = c(1, 1), common.legend = TRUE, legend = "bottom")
```
...
eval_ihw_betamix_noise_bocaleek <- fdp_eval_ihw(strat = "BocaLeek", ihw_res = ihw_betamix_noise_BocaLeek, sim = betamix_sim)
...
eval_ihw_betamix_noise_RFCDE <- fdp_eval_ihw(strat = "RFCDE", ihw_res = ihw_betamix_noise_RFCDE, sim = betamix_sim)
##      rjs       pow        FDR
## 1 471.17 0.4225245 0.08725322
```
...
fdp_eval_list(ihw_univ_BocaLeek, Hs)
##     rjs        pow        FDR
...
fdp_eval_list(ihw_univ_RFCDE, Hs)
##     rjs        pow        FDR
## 1 64.59 0.05847762 0.09265819
```
