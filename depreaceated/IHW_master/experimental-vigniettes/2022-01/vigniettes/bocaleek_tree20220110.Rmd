...
Loading my current status of the code (work in progress...). Would there be a more elegant way, to test my code without building the package every time?
```{r load_functions, echo = F, , message = F}
...
ihw_quantile_slicing <- foreach(Xs_i = Xs, Ps_i = Ps) %dopar% {
  ihw(Ps_i, Xs_i, alpha = alpha, folds = folds, use_forest = FALSE, lambda = lambda)
}
fdp_eval(ihw_quantile_slicing, Hs)
```
