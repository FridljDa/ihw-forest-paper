...
doParallel::registerDoParallel(cores = min(10, n.cores - 1))
library(doRNG)
```
Now we need to download my current version of IHW. Make sure the official IHW package is not loaded.
`remove.packages("IHW")`. Make sure you have the most current version of `remotes`
...
```
https://github.com/FridljDa/IHW/tree/main/bioconductor
...
To run the original IHW (quantile slicing), we need to manually bin the covariates in 2d squares
```{r manually bin Xs_i}
...
Next we benchmark on the airway data set, also used in the official vigniette: http://bioconductor.org/packages/release/bioc/vignettes/IHW/inst/doc/introduction_to_ihw.html
...
sum(IHW::rejected_hypotheses(ihw_airway_slic), na.rm = T)/m
```
...
sum(IHW::rejected_hypotheses(ihw_airway_forest), na.rm = T)/m
```
