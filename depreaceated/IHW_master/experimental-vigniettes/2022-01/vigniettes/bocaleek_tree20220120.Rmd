...
We run the normal IHW with the binned covariates. The option `use_forest = FALSE` makes it equivalent to the original IHW.
...
Ps_i <- Ps[[1]]
Xs_i <- Xs[[1]]
#Xs_i_strat <- group_by_forest2(pvalues = Ps_i, covariates = Xs_i, folds, nbins = 16, ntrees = ntrees, seed = NULL)
ihw_forest <- ihw(Ps_i, Xs_i, alpha = alpha, folds = folds, use_forest = TRUE, ntrees = ntrees, nbins=20 ,tau = tau, lambda = lambda)
```
Now we run IHW with forest. 
```{r run_IHW_forest, message=FALSE}
ihw_betamix_forest <- foreach(Xs_i = Xs, Ps_i = Ps) %dopar% {
  Xs_i_strat <- group_by_forest2(pvalues = Ps_i, covariates = Xs_i, folds, nbins = 16, ntrees = ntrees, seed = NULL)
  ihw_forest <- ihw(Ps_i, Xs_i_strat, alpha = alpha, folds = folds, use_forest = TRUE, ntrees = ntrees, tau = tau, lambda = lambda)
}
...
ihw_univ_forest <- foreach(Xs_i = Xs, Ps_i = Ps) %dopar% {
  Xs_i_strat <- group_by_forest2(pvalues = Ps_i, covariates = Xs_i, folds, nbins = 16, ntrees = ntrees, seed = NULL)
  ihw(Ps_i, Xs_i_strat, alpha = alpha, folds = folds, use_forest = TRUE, ntrees = ntrees, tau = tau, lambda = lambda)
}
