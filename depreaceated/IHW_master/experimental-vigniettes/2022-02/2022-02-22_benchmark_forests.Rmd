...
devtools::load_all() 
devtools::load_all("../../../covariate-powered-cross-weighted-multiple-testing/IHWStatsPaper")
#library("IHW")
```
...
```{r, eval = F}
ihw_airway_Julia <- ihw(pvalue ~ baseMean, data = deRes, alpha = alpha, folds = folds, strat = "Julia", ntrees = ntrees, tau = tau, lambda = lambda)
#ihw(pvalue, covariate_noise, folds =folds,alpha = 0.1, strat = "Julia", ntrees = ntrees, lambda = lambda, nbins = nbins, nbasis = nbasis)
rejections(ihw_airway_Julia)
```
