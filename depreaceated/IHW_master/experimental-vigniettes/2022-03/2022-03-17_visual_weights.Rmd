...
taus = taus,  n_censor_thres = n_censor_thres, maxdepth = maxdepth, min.node.size = min.node.size, lambda = Inf, folds = folds_fdp_eval)
...
```{r}
groups_bocaleek <- group_by_forest_BocaLeek(data_train_test$pvalue, select(data_train_test, starts_with("cov")), folds, nbins=nbins, ntrees = ntrees, taus = taus, n_censor_thres = n_censor_thres, maxdepth = maxdepth, min.node.size = min.node.size) 
