...
```
```{r}
groups_bocaleek <- group_by_forest_BocaLeek(pvalues, as.matrix(covariates), folds, ntrees = ntrees, taus = taus, n_censor_thres = n_censor_thres, maxdepth = maxdepth, min.node.size = min.node.size, nsplit = nsplit)
str(groups_bocaleek)
```
...
ihw_bocaleek <- ihw(pvalues, covariates,covariate_type = "ordinal",nbins == "auto",
    alpha = 0.1, strat = "BocaLeek",folds, ntrees = ntrees, taus = taus, n_censor_thres = n_censor_thres, maxdepth = maxdepth, min.node.size = min.node.size, nsplit = nsplit, lambda = Inf)
