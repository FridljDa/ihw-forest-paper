...
```{r}
sim_res <- purrr::map_dfr(data_sim, function(data_sim_i){
  ...
  s_line_t_test_lm <- s_line_t_test_lm %>%
    mutate(residual_sigma = summary(t_test_lm)$sigma,
           snr = Estimate/residual_sigma,
           #calculate minor_allele_frequency
           minor_allele_frequency = 0.5* min(mean(data_sim_i$s), 2- mean(data_sim_i$s)), 
           s_variance = var(data_sim_i$s))
  
  s_line_t_test_lm
})

sim_res <- sim_res %>% rename(pvalue = `Pr...t..`)
sim_res <- sim_res %>% rename(betha_estimate_std = `Std..Error`)
...
#ggpairs(flea, columns = 2:3,
#        lower=list(continuous=wrap(GGscatterhex)))
```
...
ggplot(sim_res, 
       aes(x = pvalue,
           fill = H)) +
  geom_histogram(boundary = 0) +
  facet_wrap(groups_by_filter(sim_res$residual_sigma, 8), nrow = 2)
```
...
ggplot(sim_res, 
       aes(x = pvalue,
           fill = H)) +
  geom_histogram(boundary = 0) +
  facet_wrap(groups_by_filter(sim_res$s_variance, 8), nrow = 2)
```
...
ggplot(sim_res, 
       aes(x = pvalue,
           fill = H)) +
  geom_histogram(boundary = 0) +
  facet_wrap(groups_by_filter(sim_res$betha_estimate_std, 8), nrow = 2)
```
