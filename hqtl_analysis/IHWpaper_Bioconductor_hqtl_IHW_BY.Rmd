...
Rejections of IHW-BY with censoring trick:

```{r}
rejections(ihw_chr1_chr2_no_binning)
```

Rejections of IHW-BY with binning:

```{r}
rejections(ihw_chr1_chr2)
```

So we see that discoveries have more than doubled. IHW-BY with binning with pre-binning, because it can learn the features of the covariate space on the full, uncensored data set. 
...
my_breaks
```
sort of logarithmic scale, more granular with very small distances
...
```{r}
stratification_breaks(ihw_chr1_chr2_no_binning)
```
