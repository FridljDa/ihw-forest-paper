...
The [downloaded data set](https://github.com/nignatiadis/IHWpaper/blob/master/inst/extdata/real_data/hqtl_chrom1_chrom2/chr1_subset.Rds) has columns `beta` and `tstat`. `beta_se` was calculated the following way `mutate(beta_se = beta/tstat)`.
```{r}
...
```{r}
#tmp <- readRDS(here::here("data/hqtl_chrom1_chrom2/results/chr1_mod_H3K27AC"))
#chr1_pvalues_grubert_rerun <- tmp$all$eqtls 
chr1_pvalues_grubert_rerun <- read.table(here::here("data/hqtl_chrom1_chrom2/results/cisQTLs_H3K27AC_chr1.txt"), header = TRUE, stringsAsFactors = FALSE)
head(chr1_pvalues_grubert_rerun)
```

```{r}
chr1_pvalues_grubert_rerun_join <- inner_join(chr1_pvalues_grubert,
                                              chr1_pvalues_grubert_rerun %>% mutate(gene = as.integer(gene)), 
                                              by = c("SNP","gene"))
colnames(chr1_pvalues_grubert_rerun_join)
```

```{r}
ggplot(chr1_pvalues_grubert_rerun_join,
