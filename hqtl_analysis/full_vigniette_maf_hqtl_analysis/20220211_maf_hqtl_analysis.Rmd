...
svars_bin_hist <- lapply(chr_pvalues_grubert_split_svars_bin, function(svars_bin_i){
  title <- unique(svars_bin_hist_i$svars_bin)
  title <- paste0(title, ", ")
...
#IHW
```{r}
ihw_maf_equal <- ihw(chr_pvalues_grubert$pvalue, chr_pvalues_grubert$maf_sample_bin, alpha = 0.05)
```

```{r}
ihw_maf_unequal <- ihw(chr_pvalues_grubert$pvalue, chr_pvalues_grubert$maf_bin_uneq)
```

```{r}
ihw_maf_unequal <- ihw(chr_pvalues_grubert$pvalue, chr_pvalues_grubert$svars_bin)
```
