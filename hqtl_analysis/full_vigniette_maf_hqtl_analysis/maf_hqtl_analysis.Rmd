...
ggplot(pvalues_maf_boca %>% select(pvalue), 
       aes(x = pvalue)) +
  #scale_x_log10()+
  geom_histogram(boundary = 0, bins = 50) 
```
...
```{r}
chr_pvalues_grubert <- chr_pvalues_grubert %>%
  mutate(maf_sample_bin = groups_by_filter(maf_sample, 8))
head(chr_pvalues_grubert)
```

```{r pvalue_hist_by_maf, eval = FALSE, fig.cap = "pvalue_hist_by_maf"}
ggplot(chr_pvalues_grubert %>%
         sample_n(5* 10^8)%>%
         select(pvalue, maf_sample_bin), 
       aes(x = pvalue)) +
  geom_histogram(boundary = 0, bins = 40) + #, bins = 40
  facet_grid(cols = vars(maf_sample_bin)) 
```
...
```{r pvalue_beta_std, eval = FALSE, fig.cap = "Pvalues against betha std"}
ggplot(chr_pvalues_grubert %>% select(beta_se, pvalue),
...
```{r pvalue_beta_std_boca, fig.cap = "Pvalues against betha std boca", eval = FALSE}
ggplot(pvalues_maf_boca,
       aes(x = beta_se, y =  pvalue)) +
  scale_y_log10()+
  scale_fill_gradient(name = "count", trans = "log10")+
