...
```{r, eval = TRUE}
chr21_pvalues_grubert <- chr21_pvalues_grubert %>% sample_n(1*10^5) #3000000  #1*10^5
```
...
chr21_pvalues_grubert <- chr21_pvalues_grubert %>%
  mutate(beta_se = beta/tstat, #TODO correct?
         #tstat = NULL,
         FDR = NULL)
```
...
```{r}
#more complicated because of \sqrt{s s_{x x} x}
