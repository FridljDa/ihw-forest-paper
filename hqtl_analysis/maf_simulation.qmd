...
```{r, eval = FALSE}
chr <- 21
chr_full_data_grubert <- load(paste0("/g/scb/zaugg/zaugg/hQTL/GenotypeData/snps", chr, ".filtered.allGT.txt.rda"))
snps
```
...
simulate_data <- function(H, s, nsample = 76, alpha = 50, betha = 5, sigma = 100){
  data_sim <- data.frame(
...
```{r}
length(chr_full_data_grubert)
head(chr_full_data_grubert)
chr_full_data_grubert[[1]]
```
...
data_sim <- mapply(simulate_data, H, sampled_s)
head(data_sim[[1]])
```
