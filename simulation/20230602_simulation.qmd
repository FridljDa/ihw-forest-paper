...
sim <- noise_sim(m = 10000, r = 2, dimensions = 1:3)
```
...
eval <- foreach(i = seq_along(sim),
    ...
    nbins_quantile <- 5^dimension_i
    node_size_forest <- ceiling(length(Ps_i)/nbins_quantile)
    
    ihw_quantile <- IHW::ihw(Ps_i, Xs_i, alpha = 0.1, stratification_method = "quantiles", nbins = nbins_quantile)
  ...
  effective_nbins_forest <- mean(unlist(IHW::nbins(ihw_forest)))
    ...
    tribble(
  ~stratification_method, ~rejections, ~dimension, ~effective_nbins,
  "forest", sum(rejected_hypotheses_forest),   dimension_i, effective_nbins_forest,
  "quantile", sum(rejected_hypotheses_quantile),   dimension_i,  effective_nbins_quantile)
}
```
