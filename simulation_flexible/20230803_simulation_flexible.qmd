...
Let $p\in (0,\infty)$. Let $\|x\|_p = \left(\sum{i=1}^{n} \|x_i\|_p\right)^{1/p}$ and $B_p(r) = \{x \in \mathbb{R}^d : \|x\|_p \leq r\}$. Let $s\in (0,1]$. Let $\pi_1: [-0.5,0.5]^d \rightarrow [0,1], x \mapsto 1\left(x\in B_p(r)\right)$. Fix $\bar{\pi}_1 \in (0,1)$. We choose $r$ s.t. $\int_{[-0.5,0.5]^d}\pi_1(x)dx=\bar{\pi}_1$.
