...
```{r}
simulation_evaluated %>%
  filter(kappa == 0  & alpha == 0.1 & lp_norm == 1 & target_average_alt_prob == 0.2 & ndim == 1 & beta_shape1 == 0.25 & signal_strength == 0.8) %>% 
  plot_fdr_power(group_by_dimension = "dimensions", log_trans = TRUE, silent = TRUE)
```

```{r}
simulation_evaluated %>%
  filter(kappa == 0.1  & alpha == 0.1 & lp_norm == 1 & target_average_alt_prob == 0.1 & ndim == 1 & beta_shape1 == 0.25 & signal_strength == 0.8) %>% 
  plot_fdr_power(group_by_dimension = "dimensions", log_trans = TRUE, silent = TRUE)
```
