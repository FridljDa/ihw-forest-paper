...
formulas_alpha_levels <- formulas_alpha_levels[1:2, ] # TODO
```
    ...
    forest_rej <- foreach(i = 1:nrow(formulas_alpha_levels)) %dorng% {
      ...
      ihw_forest_i <- error_handler(try(
        ihw(
          ...
          n_censor_thres = 1, ntrees = 1, nodedepth = 6, nodesize = 5000 # TODO
        )
      ))
...
```{r, eval = FALSE}
result <- bind_rows(by_rejections, quantile_rej)

hqtl_rejections<- ggplot(result, aes(x = alpha, y = rejections, shape = interaction(method, formula), col = interaction(method, formula))) +
  geom_line() +
  geom_point(size = 2) +
