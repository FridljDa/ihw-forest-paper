...
chr1_df <- left_join(chr1_df, snp_chr1, by=(c("SNP"="snp"))) %>%
       ...
       transmute(SNP, pvalue, dist = pmin(abs(pos-start), abs(pos-end)))
...
rm(chr1_more_covm, chr2_more_cov)

chr1_chr2_df <- chr1_chr2_df %>% 
  dplyr::select(-one_of(c("SNP","gene", "pos", "start","end"))) 
```
...
formulas_mt <- c("dist",
  ...
  paste(c("dist","minor_allele_freq",paste0("cov",1:10)), collapse = " + "))
...
```{r}
data.frame(formula = formulas_mt,
           quantile_rej = sapply(ihw_quantile, rejections),
           forest_rej = sapply(ihw_forest, rejections))
```
