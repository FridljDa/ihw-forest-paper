...
quantile_rej <- xfun::cache_rds(
  {
    ...
    quantile_rej <- foreach(i = 1:nrow(formulas_alpha_levels)) %dorng% {
      ...
      quantile_rej_i <- data.frame(
        ...
        nbins_covariates = nbins(ihw_quantile_i)
      )
    }
...
forest_rej <- xfun::cache_rds(
  {
    ...
    forest_rej <- foreach(i = seq_len(nrow(formulas_alpha_levels))) %dorng% {
      ...
      forest_rej_i <- data.frame(
        ...
        nbins_covariates = mean(unlist(nbins(ihw_forest_i)))
      )
    }
...
bh_rejections <- foreach(i = seq_len(nrow(alpha_levels))) %dorng% { 
  ...
  bh_rejections_i <- sum(p.adjust(na.exclude(chr1_chr2_df$pvalue), method = "BH") <= alpha_i)
  # evaluate number of rejections
  data.frame(
    method = "BH",
    formula = "pvalue ~ 1",
...
```{r}
result <- bind_rows(bh_rejections, quantile_rej, forest_rej)
...
hqtl_rejections_by_forest <- result %>% 
  ...
  #mutate(label = if_else(alpha == max(alpha), paste(method, formula_str), NA_character_)) %>%
ggplot(aes(x = alpha, y = rejections, col = paste(method, formula_str))) +
  ...
  #                na.rm = TRUE)+
 scale_x_continuous(breaks = unique(result$alpha)) +
 guides(col=guide_legend(nrow=4,byrow=TRUE, title="Method, formula"))+
 scale_y_log10() 
