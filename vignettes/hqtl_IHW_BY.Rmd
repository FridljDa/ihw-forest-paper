...
forest_rej <- xfun::cache_rds(
  {
    # run for all alpha-formula combinations (in parallel)
    forest_rej <- foreach(i = 1:nrow(formulas_alpha_levels)) %dorng% {
      print(paste0("simulation run:", i))
...
```{r by_rej}
# run for all alpha-formula combinations (in parallel)
by_rejections <- foreach(i = seq_along(alphas)) %dorng% {
  print(paste0("simulation run:", i))

  alpha_i <- alphas[[i]]
  alphas_by_i <- alphas_by[[i]]
...
result <- result %>%
  ...
  #unite("method_covariates", method:number_covariates, remove = FALSE)
# concat
```

```{r}
result %>% count(method, alpha, formula)
```


```{r}
hqtl_rejections <- ggplot(result, aes(x = alpha, y = rejections, col = method_covariates
                                      )) +
  geom_line() +
  geom_point(size = 2) +
