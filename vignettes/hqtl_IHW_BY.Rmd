...
unique(quantile_rej$formula)
nrow(quantile_rej) == nrow(formulas_alpha_levels)
```
...
result <- result %>%
  ...
  #mutate(method_covariates = ifelse(number_covariates == 1,
  #                                  paste(method, formula),
  #                                  paste(method, number_covariates, "cov")))
```

```{r, eval = FALSE}
formulas_mt[c(1,4)]
result %>% filter(formula %in% formulas_mt[3])
```

```{r}
result$formula
formula_abbreviation <- list("pvalue ~ 1",
                             "pvalue ~ dist",
                             "pvalue ~ minor_allele_freq",
                             "pvalue ~ dist + minor_allele_freq + H3K27Ac_1_snp + H3K27Ac_1_peak + ,     H3K27Ac_2_snp + H3K27Ac_2_peak + H3K27Ac_3_snp + H3K27Ac_3_peak + ,     H3K4me3_1_snp + H3K4me3_1_peak + H3K4me3_2_snp + H3K4me3_2_peak + ,     H3K4me3_3_snp + H3K4me3_3_peak + kH3k4me1_1_snp + kH3k4me1_1_peak + ,     kH3k4me1_2_snp + kH3k4me1_2_peak + kH3k4me1_3_snp + kH3k4me1_3_peak")

names(formula_abbreviation) <- list("pvalue ~ 1",
                             "pvalue ~ dist",
                             "pvalue ~ minor_allele_freq",
                             "pvalue ~ dist + minor_allele_freq",
     "pvalue ~ 20 covariates")

formula_abbreviation2 <- setNames(names(formula_abbreviation), formula_abbreviation)

x[names(y)] <- y

replace(results2$formula_str, names(formula_abbreviation2), formula_abbreviation2)

results2$formula_str[names(formula_abbreviation2)] <- formula_abbreviation2
```
...
result <- result %>%
  mutate(formula_str = ifelse(number_covariates < 20,
                              format(formula),
                              "pvalue ~ 20 covariates"),
...
# scale_x_continuous(breaks = seq(0, 1000, by = 250)) +
 scale_y_log10() +
  facet_wrap(vars(formula), labeller=formula_abbreviation)
