...
The full analysis has 15.725.016.812 = 16 billion hypotheses. This is too massive. Simply evaluating `645452 * 12193 + 699343 * 11232` leads to an Integer overflow in R, not to speak of a data.frame with that many rows. To get some progress, we need to sub-sample it. Because `chr1_df` only contains p-values <= 1e-4, the below sub-sampling is biased. We will revisit this later.
```{r, eval = F, include=F}
...
```{r, eval = T, include=T}
chr1_df <- chr1_df %>% slice_sample(n = 2000) 
chr2_df <- chr2_df %>% slice_sample(n = 2000) 
...
In [here](http://bioconductor.org/packages/release/data/experiment/vignettes/IHWpaper/inst/doc/hqtl_IHW_BY.html) only the genomic distance was used as covariate. Here, we also want to use minor allele frequency (MAF). So we are going to download them using `biomaRt`.
```{r, eval = F}
ensembl <- biomaRt::useMart("ENSEMBL_MART_SNP", dataset = "hsapiens_snp")
```
The below code is not very efficient so we have already pre-downloaded it. NA-values are not allowed for the covariates, so we set all MAFs missing in the database to 0. We download MAF for the first chromosome under study.
```{r, eval = F}
...
Now the same for the second chromosome.
```{r, eval = F}
...
We load the pre-downloaded MAF file and annotate the hypotheses accordingly.
```{r, eval = T}
...
full_comb_chr1 <- full_comb_chr1 %>% left_join(chr1_maf, by = "snp")
full_comb_chr2 <- full_comb_chr2 %>% left_join(chr2_maf, by = "snp")
```
...
```{r}
paste(100*m/15725016812,"%")
```

Again, the sub-sampling was biased. Let us put the data for the two chromosomes together:
...
Now let us run the IHW procedure with the default `"quantiles"` stratification method only using genomic distance as covariate:
```{r}
...
Using genomic distance and maf  as covariates:
```{r}
...
Run IHW with the `"forest"` stratification method.
```{r, eval = T, warning= FALSE}
ihw_forest_dist <- ihw(pvalue ~ dist, data = chr1_chr2_df,
  alpha, folds = folds_vec, stratification_method = "forest",
  n_censor_thres = 5, ntrees = 5, nodedepth = 5, nodesize = 1000
)
```
...
```{r, eval = T, warning= FALSE}
ihw_forest_dist_maf <- ihw(pvalue ~ dist + minor_allele_freq, data = chr1_chr2_df,
  alpha, folds = folds_vec, stratification_method = "forest",
  n_censor_thres = 5, ntrees = 5, nodedepth = 5, nodesize = 1000
)
```
...
```
Rejections of IHW-BY-quantile using genomic distance and maf:

```{r}
rejections(ihw_quantile_dist_maf)
```

Rejections of IHW-BY-quantile only using genomic distance:

```{r}
rejections(ihw_quantile_dist)
```

Rejections of IHW-BY-forest using genomic distance and maf:

```{r}
rejections(ihw_forest_dist_maf)
```

Rejections of IHW-BY-forest only using genomic distance:

```{r}
rejections(ihw_forest_dist)
```

So we see that the number of discoveries has increased. MAF turned out to be not informative, but decreased number of rejections for all stratification methods. However, while IHW-quantile was completely thrown of by an this extra dimension and reduced to classical BY with uniform weights, IHW-Forest was able to handle it.
