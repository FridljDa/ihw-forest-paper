...
```{r, eval = F}
snp_chr1 <- snp_chr1 %>% slice(1000)
snp_chr2 <- snp_chr2 %>% slice(1000)

peaks_chr1 <- peaks_chr1 %>% slice(1000)
peaks_chr2 <- peaks_chr2 %>% slice(1000)
```


The below sampling does the job, ensures that there are enough "real" hypotheses, but is biased
```{r, eval = T}
chr1_df <- chr1_df %>% sample_n(10000)
chr2_df <- chr2_df %>% sample_n(10000)
...
```{r}
full_comb_chr1 <- full_join(select(snp_chr1, snp, pos), peaks_chr1, by = character())
full_comb_chr2 <- full_join(select(snp_chr2, snp, pos), peaks_chr2, by = character())
```
...
strat_quantile <- stratification_breaks(ihw_quantile)
```
...
ihw_forest <- ihw(chr1_chr2_df$pvalue, chr1_chr2_df$dist, alpha,
  ...
  n_censor_thres = 2, ntrees = 1, nsplit = NULL, nodedepth = 4, nodesize = 1000
)
#IHW::m_groups(ihw_forest)
strat_forest <- stratification_breaks(ihw_forest, fold = 1, tree = 1) # , n_censor_thres = , ntrees = 1
```

```{r, eval =F}
ggplot(df1, aes(x = x, y = y)) +
  geom_line() +
  geom_vline(xintercept = strat_forest)
```
