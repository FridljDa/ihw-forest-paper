...
```{r, eval = T, include=T}
chr1_df <- chr1_df %>% slice_sample(n = 1000) # slice_sample(n = 1000)
chr2_df <- chr2_df %>% slice_sample(n = 1000) # slice_sample(n = 1000)
...
```{r, eval = F}
snp_chr1 <- snp_chr1 %>% left_join(chr1_maf, by = (c("snp" = "refsnp_id")))
snp_chr2 <- snp_chr2 %>% left_join(chr2_maf, by = (c("snp" = "refsnp_id")))
#TODO check, all downloaded
```
...
chr1_df <- full_comb_chr1 %>%
  ...
  transmute(dist, 
            #minor_allele_freq = replace_na(minor_allele_freq, 0), 
...
rm(chr1_df, chr2_df)
rm(snp_chr1, peaks_chr1, snp_chr2, peaks_chr2, full_comb_chr1, full_comb_chr2, chr1_maf, chr2_maf)
```
