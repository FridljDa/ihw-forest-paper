...
```{r, eval = T, include=T}
chr1_df <- chr1_df %>% slice_sample(n = 10000) 
chr2_df <- chr2_df %>% slice_sample(n = 10000) 
...
In [here](http://bioconductor.org/packages/release/data/experiment/vignettes/IHWpaper/inst/doc/hqtl_IHW_BY.html) only the genomic distance was used as covariate. Here, we also want to use minor allele frequency (MAF). We load the pre-downloaded MAF file and annotate the hypotheses accordingly.
```{r, eval = T}
...
full_comb_chr1 <- full_comb_chr1 %>% 
  left_join(chr1_maf, by = c("snp" = "refsnp_id")) %>%
  mutate(minor_allele_freq = replace_na(minor_allele_freq, 0))
full_comb_chr2 <- full_comb_chr2 %>% 
  left_join(chr2_maf, by = c("snp" = "refsnp_id")) %>%
  mutate(minor_allele_freq = replace_na(minor_allele_freq, 0))
```
...
```{r ihw_quantile_dist, include=TRUE, eval=TRUE}
...
```{r ihw_forest_dist, eval = T, warning= FALSE}
...
```{r ihw_forest_dist_maf, eval = T, warning= FALSE}
