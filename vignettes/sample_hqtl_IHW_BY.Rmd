...
```{r, eval = F}
chr1_maf <- data.frame(SNP = unique(full_comb_chr1$SNP),
                       minor_allele_freq = NA)

pb = txtProgressBar(min = 0, max = nrow(chr1_maf), initial = 0) 
for (i in seq_len(nrow(chr1_maf))){
  setTxtProgressBar(pb,i)
  refsnp_id <- chr1_maf$SNP[i]
  tmp <- biomaRt::getBM(
    attributes=c("minor_allele_freq"),
    filters="snp_filter", values= refsnp_id,
    mart=ensembl, uniqueRows=TRUE)
  chr1_maf$minor_allele_freq[i] <- ifelse(nrow(tmp) == 1,
                                          tmp$minor_allele_freq,
                                          0)
}
...
```{r, eval = F}
chr2_maf <- data.frame(SNP = unique(full_comb_chr2$SNP),
                       minor_allele_freq = NA)

pb = txtProgressBar(min = 0, max = nrow(chr2_maf), initial = 0) 
for (i in seq_len(nrow(chr2_maf))){
  setTxtProgressBar(pb,i)
  refsnp_id <- chr2_maf$SNP[i]
  tmp <- biomaRt::getBM(
    attributes=c("minor_allele_freq"),
    filters="snp_filter", values= refsnp_id,
    mart=ensembl, uniqueRows=TRUE)
  chr2_maf$minor_allele_freq[i] <- ifelse(nrow(tmp) == 1,
                                          tmp$minor_allele_freq,
                                          0)
}
...
any(is.na(chr2_df))
```
So compared to the full analysis, it is only a fraction:
```{r}
...
Now let us run the IHW procedure with the default `"quantiles"` stratification method:
```{r}
ihw_quantile <- ihw(chr1_chr2_df$pvalue, chr1_chr2_df$dist, alpha, folds = folds_vec)
```

```{r}
try(ihw_quantile <- ihw(chr1_chr2_df$pvalue, chr1_chr2_cov, alpha, folds = folds_vec))
```

run some diagnostics on the `ihw_quantile` object
```{r, eval = T}
...
```
So we see that the number of discoveries has increased.
